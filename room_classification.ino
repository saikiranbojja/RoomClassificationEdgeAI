...
static const char *label[] = {"Bedroom", "Classroom", "Lift"};
...
static constexpr int tensor_arena_size = 100000;
...
void tflu_initialization() {
  ...
  // static tflite::MicroInterpreter static_interpreter(tflu_model, tflu_ops_resolver, tensor_arena, tensor_arena_size, &tflu_error);
...
void setup() {
  ...
  if (!Camera.begin(QQVGA, YUV422, 5)) {
...
void loop() {
  ...
  for (int yo = 0; yo < h1; yo++) {
    ...
    for(int xo = 0; xo < w1; xo++) {
      ...
      for(int i = 0; i < 3; i++) {
        ...
        tflu_i_tensor->data.int8[idx++] = c_q;   //Input
        if(debug_application) {
          Serial.println(c_i);
        }
      }
  ...
  size_t ix_max = 0;
  float  pb_max = 0;
  Serial.print("confidence scores ");
  for (size_t ix = 0; ix < 3; ix++) {
    Serial.print(label[ix]);
    Serial.print(":");
    Serial.print(tflu_o_tensor->data.f[ix]);
    Serial.print("  ");
